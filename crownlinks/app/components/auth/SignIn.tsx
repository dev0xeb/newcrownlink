      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-8">
        ...
        <form className="space-y-5">
          <div>
            ...
            <input id="email" type="email" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-200 outline-none" placeholder="you@email.com" />
          </div>
          <div>
            <label className="block text-gray-700 mb-1" htmlFor="password">Password</label>
            <input id="password" type="password" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-200 outline-none" placeholder="Password" />
          </div>
          <button type="submit" className="w-full bg-[#1A6EDC] text-white font-semibold py-2 rounded-lg hover:bg-blue-700 transition-colors">Sign In</button>
        </form>
        <p className="mt-6 text-center text-gray-600 text-sm">
          Don't have an account?{' '}
          <Link href="/signup" className="text-[#1A6EDC] hover:underline">Sign Up</Link>
        </p>
      </div>
    </div>
  );
